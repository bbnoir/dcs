  ...
  case (state)
    ...
    ML: begin
      ...
      if(!t_done) state_nxt = ML;
        ...
        if(|car_side_s_reg || |car_side_lt_reg) state_nxt = MY;
        else state_nxt = MY;
      end
    end
    MY: begin
      {out_valid_nxt, light_main_nxt, light_side_nxt} = {1'b1, YELLOW, RED};
      if(|car_main_lt_reg) state_nxt = MR;
      else if(|car_side_s_reg) state_nxt = SR;
      else if(|car_side_lt_reg) state_nxt = SR;
      else state_nxt = MR;
    end
    MR: begin
